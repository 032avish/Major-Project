...
const MyDocument = ({ droppedItemsMap }) => {
  ...
  // Get the last seven keys from droppedItemsMap
  const lastSevenKeys = Array.from(droppedItemsMap.keys()).slice(-7);

  // Create a new map with only the last seven keys and their corresponding values
  const lastSevenItemsMap = new Map(
    lastSevenKeys.map((key) => {
  ...
  return (
    <Document>
      {Array.from(lastSevenItemsMap.keys()).map((date, index) => (
        <Page key={index} style={styles.page}>
          ...
          <View style={styles.table}>
            ...
            {lastSevenItemsMap.get(date).map((item, i) => {
